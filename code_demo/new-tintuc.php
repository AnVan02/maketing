...
if (!isset($_SESSION['blocks'])) {
    $_SESSION['blocks'] = [
        ['id' => 'single_block', 'type' => 'text', 'content' => '<h1>Tiêu đề H1</h1><h2>Tiêu đề H2</h2><h3>Tiêu đề H3</h3><h4>Tiêu đề H4</h4><h5>Tiêu đề H5</h5><p>Đoạn văn bản của bạn. Nhấn Enter để xuống dòng bình thường.</p><p>Bạn có thể viết nhiều đoạn văn trong cùng một block.</p>']
    ];
}
...
<style>
  :root{--bg:#f5f6f8;--card:#fff;--muted:#6b7280}
  *{box-sizing:border-box}
  body{margin:0;font-family:Inter, Arial, sans-serif;background:var(--bg);color:#111}
  .container{display:flex;gap:18px;height:100vh;padding:18px}
  .left{flex:1;min-width:0;padding:18px;background:transparent;overflow:auto;position: sticky}
  .editor-area{background:var(--card);padding:16px;border-radius:12px;box-shadow:0 6px 18px rgba(0,0,0,0.06)}
  .right{width:360px;padding:18px}
  .editor-header{position:sticky;top:0;background:var(--bg);z-index:100;padding:12px 0;margin-bottom:12px;border-bottom:1px solid #e5e7eb}
  h2{margin:0 0 12px 0}
  .controls{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px}
  button, input[type="button"]{cursor:pointer}
  .btn{padding:8px 12px;border-radius:8px;border:0;background:#2563eb;color:#fff}
  .btn.ghost{background:#fff;border:1px solid #e5e7eb;color:#111}
  .drop-hint{border:2px dashed #cbd5e1;padding:16px;border-radius:8px;text-align:center;color:var(--muted);margin-bottom:12px}
  .block{background:var(--card);padding:10px;border-radius:10px;margin-bottom:12px;position:relative;border:1px solid #eef2f7}
  .block.dragging{opacity:0.6}
  .block .toolbar{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px}
  .toolbar button{padding:6px 8px;border-radius:6px;border:1px solid #e6eefc;background:#fff;font-size:13px}
  .contenteditable{min-height:44px;padding:8px;border-radius:6px;outline:none;line-height:1.6}
  .close-btn{position:absolute;top:8px;right:8px;background:#ef4444;color:#fff;border:0;border-radius:6px;padding:4px 8px}
  .edit-note{font-size:12px;color:var(--muted);margin-top:6px}
  /* Block action buttons */
  .block-actions{display:flex;gap:8px;justify-content:center;padding:12px 0;margin-top:12px;border-top:1px solid #eef2f7;position:relative}
  .block-action-btn{display:flex;align-items:center;gap:6px;padding:8px 16px;border-radius:8px;border:1px solid #e5e7eb;background:#fff;color:#2563eb;cursor:pointer;font-size:14px;transition:all 0.2s;position:relative}
  .block-action-btn:hover{background:#eff6ff;border-color:#93c5fd}
  .block-action-btn .icon{font-size:16px;font-weight:bold}
  .block-action-btn[data-tooltip]:hover::after{content:attr(data-tooltip);position:absolute;bottom:100%;left:50%;transform:translateX(-50%);margin-bottom:8px;padding:6px 10px;background:#1f2937;color:#fff;border-radius:6px;font-size:12px;white-space:nowrap;z-index:1000;pointer-events:none}
  .block-action-btn[data-tooltip]:hover::before{content:'';position:absolute;bottom:100%;left:50%;transform:translateX(-50%);margin-bottom:2px;border:5px solid transparent;border-top-color:#1f2937;z-index:1000;pointer-events:none}
  /* preview column */
  .preview-list{display:flex;flex-direction:column;gap:12px}
  .preview-item{background:var(--card);padding:8px;border-radius:8px;display:flex;gap:10px;align-items:center;border:1px solid #eef2f7;cursor:grab}
  .preview-item img{width:100px;height:60px;object-fit:cover;border-radius:6px}
      ...
      <?php foreach ($_SESSION['blocks'] as $b): ?>
        <div class="block" data-id="<?php echo $b['id'] ?>">
          <div class="toolbar">
            ...
            <button onclick="formatHeading('h4')">H4</button>
            <button onclick="formatHeading('h5')">H5</button>
            <button onclick="formatBlock('formatBlock', 'p')">P</button>
    ...
    <div class="edit-panel">
      ...
      <div class="edit-section">
        ...
        <div class="font-options">
          <div class="font-option active" onclick="applyFontStyle('Arial, sans-serif')">Arial</div>
          <div class="font-option" onclick="applyFontStyle('Georgia, serif')">Georgia</div>
          <div class="font-option" onclick="applyFontStyle('Courier New, monospace')">Courier New</div>
          <div class="font-option" onclick="applyFontStyle('Inter, Arial, sans-serif')">Inter</div>
        </div>
      </div>
      ...
      <div class="edit-section">
        ...
        <div class="color-picker">
          <div class="color-option active" style="background:#2563eb;" onclick="applyThemeColor('#2563eb')"></div>
          <div class="color-option" style="background:#dc2626;" onclick="applyThemeColor('#dc2626')"></div>
          <div class="color-option" style="background:#16a34a;" onclick="applyThemeColor('#16a34a')"></div>
          <div class="color-option" style="background:#9333ea;" onclick="applyThemeColor('#9333ea')"></div>
          <div class="color-option" style="background:#ea580c;" onclick="applyThemeColor('#ea580c')"></div>
        </div>
      </div>
      ...
      <div class="edit-section">
        ...
        <div class="edit-option">
          <input type="checkbox" id="spellCheck" onchange="toggleSpellCheck(this.checked)">
          <label for="spellCheck">Kiểm tra chính tả</label>
        </div>
        <div class="edit-option">
          <input type="checkbox" id="lineNumbers" onchange="toggleLineNumbers(this.checked)">
          <label for="lineNumbers">Hiện số dòng</label>
        </div>
      </div>
      ...
      <div class="edit-section">
        ...
        <div class="action-buttons">
          ...
          <button class="action-btn" onclick="importContent()">
            📤 Nhập file
          </button>
          <button class="action-btn danger" onclick="resetToDefault()">
            🔄 Mặc định
          </button>
        </div>
      </div>
...
function formatBlock(cmd, value = null){
  ...
  if(autoSaveEnabled) saveEdit('single_block', el.innerHTML);
}
...
function applyThemeColor(color){
  // Apply to headings and other elements
  const el = document.querySelector('.contenteditable');
  const content = el.innerHTML;
  
  // Simple color application - in real implementation you'd want more sophisticated approach
  el.focus();
...
function toggleAutoSave(enabled){
  ...
  showToast(enabled ? 'Đã bật tự động lưu' : 'Đã tắt tự động lưu');
}

function toggleSpellCheck(enabled){
  const el = document.querySelector('.contenteditable');
  el.spellcheck = enabled;
  showToast(enabled ? 'Đã bật kiểm tra chính tả' : 'Đã tắt kiểm tra chính tả');
}

function toggleLineNumbers(enabled){
  // This would require a more complex implementation for line numbers
  showToast(enabled ? 'Đã bật hiển thị số dòng' : 'Đã tắt hiển thị số dòng');
}

function clearContent(){
  if(confirm('Bạn có chắc muốn xóa toàn bộ nội dung?')) {
    const el = document.querySelector('.contenteditable');
    el.innerHTML = '<p>Bắt đầu viết...</p>';
    if(autoSaveEnabled) saveEdit('single_block', el.innerHTML);
    showToast('Đã xóa toàn bộ nội dung');
  }
}
...
function resetToDefault(){
  if(confirm('Bạn có chắc muốn reset về mặc định?')) {
    const el = document.querySelector('.contenteditable');
    el.innerHTML = '<h1>Tiêu đề H1</h1><h2>Tiêu đề H2</h2><h3>Tiêu đề H3</h3><h4>Tiêu đề H4</h4><h5>Tiêu đề H5</h5><p>Đoạn văn bản của bạn. Nhấn Enter để xuống dòng bình thường.</p><p>Bạn có thể viết nhiều đoạn văn trong cùng một block.</p>';
    if(autoSaveEnabled) saveEdit('single_block', el.innerHTML);
...
function handleTextSelection() {
  ...
  if (!block || selectedText.length === 0) {
    selectedTextData = { text: '', blockId: '', range: null };
    return;
  }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('Editor initialized with H1-H5 headings and edit panel');
});
</script>
</body>
</html>