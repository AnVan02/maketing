...
if (!isset($_SESSION['blocks'])) {
    $_SESSION['blocks'] = [
        ['id' => 'single_block', 'type' => 'text', 'content' => '<p></p>']
    ];
}
...
// Define AI quick actions
$aiQuickActions = [
    'shorten' => ['name' => 'Làm ngắn gọn', 'prompt' => 'Làm cho đoạn văn này ngắn gọn hơn nhưng vẫn giữ nguyên ý chính'],
    'expand' => ['name' => 'Mở rộng', 'prompt' => 'Mở rộng và phát triển thêm ý cho đoạn văn này'],
    'improve' => ['name' => 'Cải thiện', 'prompt' => 'Cải thiện văn phong và làm cho đoạn văn này hay hơn'],
];
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Image delete button - SỬA LẠI: LUÔN HIỂN THỊ */
        .img-delete-btn {
            position: absolute;
            top: -8px;
            right: -8px;
        ...
        .contenteditable .image-container.inline {
            display: inline-block;
            margin: 0 4px;
        }
    </style>
...
<body>
            ...
            <div class="editor-area" id="blocks">
                ...
                <?php foreach ($_SESSION['blocks'] as $b): ?>
                    <div class="block" data-id="<?php echo $b['id'] ?>">
                        <div class="toolbar">
                            <button onclick="formatBlock('formatBlock', 'h1')">H1</button>
                            <button onclick="formatBlock('formatBlock', 'h2')">H2</button>
                            <button onclick="formatBlock('formatBlock', 'h3')">H3</button>
                            <button onclick="formatBlock('formatBlock', 'h4')">H4</button>
                            <button onclick="formatBlock('formatBlock', 'h5')">H5</button>
    ...
    <script>
        ...
        const aiQuickActions = {
            'shorten': {
                name: 'Làm ngắn gọn',
                prompt: 'Làm cho đoạn văn này ngắn gọn hơn nhưng vẫn giữ nguyên ý chính'
            },
            'expand': {
                name: 'Mở rộng',
                prompt: 'Mở rộng và phát triển thêm ý cho đoạn văn này'
            },
            'improve': {
                name: 'Cải thiện',
                prompt: 'Cải thiện văn phong và làm cho đoạn văn này hay hơn'
            },
        };
        ...
        /* ---------- Update toolbar state like Word ---------- */
        function updateToolbarState() {
            ...
            const blockElement = container.nodeType === 3 ? container.parentElement : container;
            ...
            let currentBlock = blockElement;
            while (currentBlock && currentBlock !== document.body) {
                const tagName = currentBlock.tagName.toLowerCase();
                if (['h1', 'h2', 'h3', 'h4', 'h5', 'p'].includes(tagName)) {
                    // Highlight the corresponding button
                    const button = document.querySelector(`.toolbar button[onclick*="${tagName}"]`);
                    if (button) {
            ...
            if (document.queryCommandState('bold')) {
                const boldBtn = document.querySelector('.toolbar button[onclick*="bold"]');
                if (boldBtn) {
            ...
            if (document.queryCommandState('italic')) {
                const italicBtn = document.querySelector('.toolbar button[onclick*="italic"]');
                if (italicBtn) {
            ...
            if (document.queryCommandState('underline')) {
                const underlineBtn = document.querySelector('.toolbar button[onclick*="underline"]');
                if (underlineBtn) {
            ...
            if (document.queryCommandState('insertUnorderedList')) {
                const listBtn = document.querySelector('.toolbar button[onclick*="insertUnorderedList"]');
                if (listBtn) {
            ...
            if (document.queryCommandState('insertOrderedList')) {
                const listBtn = document.querySelector('.toolbar button[onclick*="insertOrderedList"]');
                if (listBtn) {
                    listBtn.classList.add('active');
                }
            }
        ...
        /* ---------- Add event listeners to contenteditable ---------- */
        function initContentEditableEvents() {
            const contentEditable = document.querySelector('.contenteditable');

            contentEditable.addEventListener('click', updateToolbarState);
            contentEditable.addEventListener('keyup', updateToolbarState);
            contentEditable.addEventListener('mouseup', function() {
        ...
        /* ---------- Image functions with delete button ---------- */
        function wrapImageWithDeleteButton(imgHTML) {
            ...
            // Copy all attributes from original img to new img
        ...
        function processNewImages() {
            const el = document.querySelector('.contenteditable');
            const images = el.querySelectorAll('img:not(.processed)');
        ...
        // Observer để tự động xử lý ảnh mới
        ...
        function insertSampleImage() {
            ...
            const imgHTML = `<img src="${randomImage}" alt="Ảnh mẫu" style="max-width: 100%; height: auto; border-radius: 8px;">`;
            const wrappedImage = wrapImageWithDeleteButton(imgHTML);

            document.execCommand('insertHTML', false, wrappedImage);
            saveEdit('single_block', el.innerHTML);
        ...
        function addImageByUrl() {
            ...
            const imgHTML = `<img src="${url}" alt="Ảnh từ URL" style="max-width: 100%; height: auto; border-radius: 8px;">`;
            const wrappedImage = wrapImageWithDeleteButton(imgHTML);
        ...
        document.getElementById('btnPreview').addEventListener('click', () => {
            ...
            // Remove delete buttons from preview
            content = content.replace(/<button class="img-delete-btn"[^>]*>×<\/button>/g, '');
            content = content.replace(/<div class="image-container"/g, '<div');
  ...
  `);
        ...
        function showAiContextMenu(e, sourceEditable) {
            ...
            if (aiMenuItem) {
                if (hasSelectedText) {
                    ...
                    aiMenuItem.innerHTML = '<span>🤖</span><span>Chat AI</span>';
                } else {
        ...
        document.querySelectorAll('.preview-item').forEach(pi => {
            pi.addEventListener('dblclick', () => {
                ...
                const imgHTML = `<img src="${url}" alt="Ảnh từ thư viện" style="max-width: 100%; height: auto; border-radius: 8px;">`;
                const wrappedImage = wrapImageWithDeleteButton(imgHTML);
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Khởi tạo observer để tự động xử lý ảnh mới
            initImageObserver();

            // Khởi tạo toolbar events
            initContentEditableEvents();
            ...
            console.log('Editor đã khởi tạo thành công! Nút xoá ảnh hoạt động ngay lập tức!');
        });
    </script>
</body>
