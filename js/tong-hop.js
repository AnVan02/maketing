...
async function collectPostMetrics(draft_post_id) {
    try {
        ...
        console.error(`❌ Lỗi thu thập chỉ số cho bài ${dreft_post_id}:`, error);
        ...
        if (currentTab === 'seo') {
            ...
            row.innerHTML = `
                ...
                <td style="text-align: center;">
                    <div style="display: flex; gap: 10px; justify-content: center;">
                        ${viewLink}
                        <!--<button class="action-btn-mini" style="color: #9ca3af !important; cursor: not-allowed;" onclick="alert('Chức năng metrics đang được cập nhật')" title="Chưa hỗ trợ">
...
window.editArticle = (id) => {
    if (currentTab === 'seo') {
        window.location.href = `viet-bai-seo.php?draft_post_id=${draft_post_id}`;
    } else {
        // Chuyển hướng sang trang cấu hình facebook với id bài viết nháp
        window.location.href = `cau-hinh-facebook.php?draft_post_id=${dreft_post_id}`;
    }
};
...
window.deleteArticle = async (id) => {
    ...
    try {
        ...
        if (currentTab === 'seo') {
            ...
            // API xoá bài nháp facebook - SỬA: dùng id thay vì draft_post_id
            url = `/facebook/publish/posts/drafts/${id}`; // Sửa ở đây
        }
...
async function updateStats(startDate = null, endDate = null) {
    try {
        ...
        if (currentTab === 'facebook') {
            updateFacebookAnalytics(); // Có thể truyền date vào đây nếu cần
        }
...
async function initCharts() {
    ...
    // ========================================
    // 2. BIỂU ĐỒ FACEBOOK - MỨC ĐỘ QUAN TÂM THEO GIỜ (HEATMAP STYLE)
    // ========================================
    await window.renderFacebookChart();
}
