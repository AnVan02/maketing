...
async function searchImages() {
    ...
    try {
        ...
        data.images.forEach(img => {
            const imgWrap = document.createElement("div");
            imgWrap.className = 'grid-image-item';
            imgWrap.style.cursor = "pointer";
            imgWrap.style.position = 'relative';

            // Normalize URL
            let src = img.image_url || img.image || img.url || '';
            if (src.startsWith('//')) src = window.location.protocol + src;
            if (src.startsWith('http://')) src = src.replace('http://', 'https://'); // prefer https

            const imgEl = document.createElement("img");
            imgEl.src = src;
            imgEl.alt = img.title || img.alt || '';
            imgEl.title = img.title || '';
            imgEl.style.width = "100%";
            imgEl.style.borderRadius = "8px";
            imgEl.style.objectFit = "cover";

            // Make draggable from media grid (attach to both image and wrapper)
            const setDragData = (e) => {
                const html = createDraggableImageHtml(src, img.title || '');
                try {
                    e.dataTransfer.setData('text/html', html);
                    e.dataTransfer.setData('text/plain', src);
                    // Use image as drag feedback if available
                    try { e.dataTransfer.setDragImage(imgEl, imgEl.width / 2 || 20, imgEl.height / 2 || 20); } catch (d) { }
                    e.dataTransfer.effectAllowed = 'copy';
                } catch (ex) {
                    try { e.dataTransfer.setData('text/plain', src); } catch (ignore) { }
                }
            };
            imgEl.draggable = true;
            imgEl.addEventListener('dragstart', setDragData);
            imgWrap.draggable = true;
            imgWrap.addEventListener('dragstart', setDragData);

            // Overlay for status
            const overlay = document.createElement('div');
            overlay.className = 'image-overlay';
            overlay.textContent = 'Click để chèn';
            ...
            // click để chèn vào editor
            imgEl.onclick = () => insertImageToEditor(src, img.title || '');

            // onerror fallback
            imgEl.onerror = () => {
                console.warn("⚠️ Image failed to load:", imgEl.src);
                const wrapper = imgEl.closest('.draggable-image, .grid-image-item');
...
function insertImageToEditor(url, title = '') {
    insertImageToActiveSection(url, title || 'Inserted Image');
}

// ============================================================
// SIDEBAR SMART EDIT SETUP
// ============================================================
function setupSidebarSmartEdit() {
    const tab = document.getElementById('smart-edit-tab');
    if (!tab) return;

    const sendBtn = tab.querySelector('.send-btn');
    ...
    const perform = async () => {
        ...
        // 1. Get Context (Active Section Content)
        let contextText = "";
        const activeSection = document.querySelector('.section-item.active .content-editor');
        if (activeSection) {
            contextText = activeSection.innerText.trim();
        }

        if (!contextText) {
            showNotification("Hãy mở một mục bài viết để AI có nội dung tham khảo!", "warning");
            return;
        }
        ...
        try {
            ...
            const payload = {
                action: val,
                selection: contextText // Send entire section content as context
            };
            ...
            if (data.success && data.rewritten_content) {
                ...
                let cleanContent = data.rewritten_content.replace(/\n/g, '<br>');
                const aiMsgHTML = `
                    <div class="ai-message">
                        <div class="ai-avatar"><img src="./images/1.png" alt="AI"></div>
                        <div class="message-bubble">
                            ${cleanContent}
                            <div style="margin-top:8px; padding-top:8px; border-top:1px dashed #cbd5e1; display:flex; gap:10px;">
                                <button class="action-btn-sm" style="color:#2563EB; border:none; background:none; cursor:pointer; font-size:12px;" onclick="navigator.clipboard.writeText(\`${data.rewritten_content.replace(/`/g, '\\`').replace(/\$/g, '\\$')}\`); alert('Đã sao chép!');">📋 Copy</button>
                                <button class="action-btn-sm" style="color:#16A34A; border:none; background:none; cursor:pointer; font-size:12px;" onclick="replaceActiveContent(\`${data.rewritten_content.replace(/`/g, '\\`').replace(/\$/g, '\\$')}\`)">⚡ Áp dụng vào bài</button>
                            </div>
                        </div>
                    </div>
                `;
            ...
            const errorHTML = `
                <div class="ai-message">
                    <div class="ai-avatar"><img src="./images/1.png" alt="AI"></div>
                    <div class="message-bubble" style="color:red;">Xin lỗi, tôi gặp lỗi khi xử lý yêu cầu này.</div>
                </div>
            `;
    ...
    // Helper to replace content in editor
    window.replaceActiveContent = (newContent) => {
        const activeSection = document.querySelector('.section-item.active .content-editor');
        if (activeSection) {
            activeSection.innerHTML = newContent; // Or use more complex logic to preserve formatting if needed
            showNotification("Đã cập nhật nội dung!", "success");
        } else {
            showNotification("Không tìm thấy mục đang mở để thay thế", "warning");
        }
    };

    sendBtn.addEventListener('click', perform);
